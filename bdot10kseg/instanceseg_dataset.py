...
class BDOT10kInstanceSeg(BDOT10kDataset):
    """BDOT10k dataset"""
    def __init__(self, **kwargs):  
        super(BDOT10kInstanceSeg, self).__init__(**kwargs)
    ...
    def get_label(self, df, idx):
        ...
        classes, bboxes, areas, segmentations = [], [], [], []
        if df is not None:
        ...
        for segmentation in segmentations:
            segmentation[:,1] = self.H-segmentation[:,1] # quick fix

        label = {
            "segmentation": np.array(segmentations),
    ...
    def plot_sample(self, img, mask, show=False):
        ...
        if classes.shape[0] != 0:
            ...
            for cls, segmentation in zip(classes, segmentation):
                img = cv2.polylines(img.copy(), [segmentation.astype(int)], True, self.colors[int(cls.item())], thickness=3)
